...
@cache.cached(timeout=90, key_prefix=lambda: f"get_data_{request.args.get('type')}")  # type: ignore
def get_data():
    ...
    if token is None:
        return (
            jsonify({"error": "Unable to authenticate and retrieve token"}),
            500,
        )
    ...
    api_response = requests.post(
        API_URL, data=data_payload, headers=headers, timeout=10
    )
...
def update_routes(routes_data):
    ...
    if os.path.exists(geojson_routes_path):
        with open(geojson_routes_path, "r", encoding="utf-8") as f:
            ...
            existing_route_ids = {
                feature["properties"]["id"] for feature in existing_geojson["features"]
            }
    else:
...
def update_objects(objects_data):
    ...
    for object in objects_data:
        ...
        if ident not in existing_objects_ids:
            ...
            features.append(
                geojson.Feature(
                    geometry=geometry, properties={"name": name, "id": ident}
                )
            )
    ...
    if features != []:
        for track in gpx.tracks:
            for segment in track.segments:
                ...
                geometry = geojson.LineString(coordinates)
                features.append(
                    geojson.Feature(geometry=geometry, properties=route_properties)
                )
    feature_collection = geojson.FeatureCollection(features)
    return feature_collection
